...
function build_results_table(ga_results){
  ...
  //check the kind of GA run, 'regular' or new breakaway?
  let ga_type = "TEAM_PURSUIT";
  if(typeof(chosen_global_settings.race_type) != "undefined"){
    ga_type = chosen_global_settings.race_type;
  }
  ...
  if(ga_type == "BREAKAWAY"){
    ...
    for(g=(ga_results.generations.length-1);g>=0;g--){
      ...
      results_html += "<tr><td style='background-color:#aaaaaa;' ondblclick=\"loadSingleRace('"+ ga_results.generations[g].final_best_race_start_order+"','"+ JSON.stringify(ga_results.generations[g].final_best_race_rider_updates_genotype).replace(/"/g, 'QQ') +"','"+g+": "+ga_results.generations[g].best_race_id+ "(" + ga_results.generations[g].best_race_time +")"+ "')\" onmouseover=\"showColName('Generation')\"><strong>" + g + "</strong></td><td onmouseover=\"showColName('Average Race Time')\"> " + ga_results.generations[g].stats_average_time + "</td><td onmouseover=\"showColName('Average Number of Instructions per race')\">" + ga_results.generations[g].stats_average_number_of_instructions + "</td><td onmouseover=\"showColName('Populaton index/ID')\">" + ga_results.generations[g].final_best_race_properties_index + "/" + ga_results.generations[g].best_race_id + "</td><td style='background-color:#aaffaa' onmouseover=\"showColName('Best Race Time (distance 2nd last/last timestep)')\">" + ga_results.generations[g].best_race_time + "/" + ga_results.generations[g].best_evolving_rider_fitness  + "</td><td onmouseover=\"showColName('Best race Start Order')\"> [" + ga_results.generations[g].final_best_race_start_order + "]</td><td onmouseover=\"showColName('Best Race Instructions')\">" + JSON.stringify(ga_results.generations[g].final_best_race_rider_updates_genotype) + "</td><td onmouseover=\"showColName('Noise alterations applied during race')\">" + JSON.stringify(ga_results.generations[g].best_race_instruction_noise_alterations) + "</td><td onmouseover=\"showColName('Run race in game model')\"><a  target='_blank' href = 'tpgame.html?source=ga&settings_id=" + selected_settings_id + "&startorder=" + encodeURI(ga_results.generations[g].final_best_race_start_order) + "&final_best_race_rider_updates_genotype=" + encodeURI(JSON.stringify(ga_results.generations[g].final_best_race_rider_updates_genotype)) + "'> Run </a></td>";
